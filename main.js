...
function getICal(urlOrFile, user, pass, sslignore, calName, cb) {
  ...
  if (!urlOrFile.match(/^https?:\/\//)) {
    ...
    request(options, (err, r, _data) => {
      if (err || !_data) {
        adapter.log.warn(
          'Error reading from URL "' + urlOrFile + '": ' + (err && err.code === "ENOTFOUND" ? "address not found!" : err)
        );
    ...
    if (ev.summary !== undefined && ev.type === "VEVENT" && ev.start && ev.start instanceof Date) {
      ...
      if (ev.rrule !== undefined) {
        ...
        try {
          ...
          adapter.log.error(
            "Issue detected in RRule, event ignored; Please forward debug information to iobroker.ical developer: " +
              e.stack +
              "\n" +
              "RRule object: " +
              JSON.stringify(rule) +
        ...
        if (dates.length > 0) {
          for (let i = 0; i < dates.length; i++) {
            ...
            adapter.log.debug(
              "   " + i + ": Event (" + JSON.stringify(ev2.exdate) + "):" + ev2.start.toString() + " " + ev2.end.toString()
            );
    ...
    if (
      ...
      adapter.log.debug(
        "Event (full day) " +
          ...
          " at " +
          ev.start.toString() +
          " filtered out, does not belong to specified time interval"
      );
    }
  } else {
  ...
  if (adapter.config.colorize) {
    ...
    // start time is in the past
      ...
      if (adapter.config.everyCalOneColor) {
        ...
        result.suffix +=
          '<span style="font-weight: normal' + (adapter.config.defColor ? "; color:" + adapter.config.defColor : "") + '">';
      }
    ...
    if (reason.includes(ev.name) || (ignoreCaseInEventname && reason.toLowerCase().includes(ev.name.toLowerCase()))) {
      ...
      if (
        ((!ev.type || ev.type === "today") &&
          event.end.getTime() > today.getTime() + ev.day * oneDay &&
          event.start.getTime() < today.getTime() + ev.day * oneDay + oneDay) ||
...
function initEvent(name, display, day, type, id, on, off, callback) {
  ...
  adapter.getState(stateName, (err, state) => {
    if (err || !state) {
      obj.state = false;
      adapter.setState(stateName, { val: false, ack: true }, () => setState(id, off, () => callback && callback(name)));
    } else {
      obj.state = state.val;
        ...
        for (let i = 0; i < adapter.config.events.length; i++) {
          for (let j = 0; j < states.length; j++) {
            ...
            if (
              ...
              if (states[j].native.enabled == event.enabled && states[j].native.display == event.display) {
    ...
    if (adapter.config.replaceDates) {
      ...
      if (daysleft === 42) {
        ...
        if (adapter.config.language === "ru") {
          ...
          text =
            (_("still") !== " " ? _("still") : "") + " " + hoursleft + " " + (hoursleft === 1 ? _("hour") : _("hours")) + _left;
        }
      } else {
        //if (adapter.config.language === 'ru') {
        // Todo: Russian
        ...
        text =
          ...
          minutesleft +
          " " +
          (minutesleft === 1 ? _("minute") : _("minutes")) +
          _left;
        //}
      }
    } else {
...
function displayDates() {
  ...
  for (let j = 0; j < events.length; j++) {
    if (!events[j].processed && events[j].state) {
      ...
      adapter.setState(name, { val: false, ack: true }, () => setState(ev.id, ev.off, retFunc));
    }
  }
}
...
function brSeparatedList(datesArray) {
  ...
  for (let i = 0; i < datesArray.length; i++) {
    ...
    adapter.log.info("ORGA: ", datesArray[i].organizer);
...
function main() {
  normal =
    ...
    (adapter.config.defColor ? "; color: " + adapter.config.defColor : "") +
    '"><span class="icalNormal">';
